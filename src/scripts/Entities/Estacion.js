...
class Estacion {
  ...
  ObtenerRenderNivelOBomba(signal, modulo) {
    ...
    const carpetaTipoSignal =
      signal.TipoSignal == EnumTipoSignal.Nivel ? "l" : "b";
    const indiceImagen =
      signal.TipoSignal == EnumTipoSignal.Nivel
        ? signal.DentroRango
          ? signal.DentroLimite == EnumDentroLimite.Alto
            ? "10r"
            : signal.IndiceImagen
          : "r"
        : signal.DentroRango
        ? signal.Valor
        : 0;
    const url = `${Core.Instance.ResourcesPath}Sitios/${
      this.Abreviacion
    }/${modulo}/${carpetaTipoSignal}/${
      EnumTipoSignalNomenclatura[signal.TipoSignal]
