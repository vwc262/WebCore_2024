import { VwcApp } from "../TablaSimplificada/App.js";
...
class Core {
   ...
   */
  get ResourcesPath() {
    return `https://virtualwavecontrol.com.mx/RecursosWeb/WebCore24/${
    ...
    this.data.forEach((estacion) => {
      ...
      estacion.signals.forEach((signal) => {
        ...
        if (signal.TipoSignal == EnumTipoSignal.Nivel) {
          ...
          signal.TipoSignal == EnumTipoSignal.PerillaBomba ||
          signal.TipoSignal == EnumTipoSignal.PerillaGeneral
        ) {
          signal.Valor = ss > 40 ? 2 : ss > 20 ? 1 : 0;
        } else if (signal.TipoSignal == EnumTipoSignal.Voltaje) {
          signal.Valor = (ss * 15.0) / 60.0;
        } else if (
          signal.TipoSignal == EnumTipoSignal.FallaAC ||
          signal.TipoSignal == EnumTipoSignal.PuertaAbierta
        ) {
