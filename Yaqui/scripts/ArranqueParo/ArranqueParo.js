...
class ArranqueParo {
  ...
  CrearCarrusel() {
    ...
    if (estacion.Lineas.length > 1) {
      ...
      let bombas = estacion.ObtenerSignalPorTipoSignal(EnumTipoSignal.Bomba);
      let idSignals = [];

      if (this.idEstacion == 1) {
        idSignals = [14, 15, 16, 17, 18, 19, 20, 21, 22];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }
      else if (this.idEstacion == 2) {
        idSignals = [4, 5];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }
      else if (this.idEstacion == 6) {
        idSignals = [72, 73, 74, 85, 86, 87, 114, 115];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }
      else if (this.idEstacion == 7) {
        idSignals = [58, 59, 60, 85, 86, 87, 114, 115];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }
      else if (this.idEstacion == 8) {
        idSignals = [58, 59, 60, 72, 73, 74, 114, 115];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }
      else if (this.idEstacion == 14) {
        idSignals = [58, 59, 60, 72, 73, 74, 85, 86, 87];
        bombas = bombas.filter(s => !idSignals.includes(s.IdSignal));
      }

      this.CrearItemsCarrusel(bombas);
    }
  ...
  CrearItemsCarrusel(bombas) {
    ...
    bombas.forEach((bomba, index) => {
      ...
      carruselItem.mySignal.myOrdinal = index;
  ...
  ArmarCodigo() {
    ...
    return (
      (_idEstacion << 8) |
      (this.#bombaSeleccionada.myOrdinal << 4) |
  ...
  SetSeleccionado(ContainerImagenBomba) {
    ...
    if (this.#carruselContainer.children.length > 3) {
      if (ContainerImagenBomba.style.left == "100px") {
        ...
        this.#bombaSeleccionada.myOrdinal = ContainerImagenBomba.mySignal.myOrdinal;
      }
    } else {
      ContainerImagenBomba.classList.add("midItem");
      ContainerImagenBomba.append(hologram);
